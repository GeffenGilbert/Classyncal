...
class ClassMeeting(BaseModel):
    ...
    days_of_week: list[Literal[
        ...
        "separate meeting per day. List only weekdays the syllabus actually states this "
        "meeting happens on. If its days are not stated anywhere, the meeting does not "
        "belong in this list at all. Never empty. A weekday that appears only once in a "
        "dated schedule is a one-off session, not a day this meeting recurs on."
    )
...
class Reading(BaseModel):
    ...
    cover that day is a topic, not a reading - including when a chapter or unit reference
    is written into that topic label. Record a reading only where the syllabus presents
    something to read: a reading column, or wording directing the student to read it. A
    course stating it has no required text usually has none."""
